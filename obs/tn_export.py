...
TERMSURL = 'https://api.unfoldingword.org/obs/txt/1/en/kt-en.json'
...
class Stories(list):
    ...
    def __init__(self):
        ...
        response = urllib.urlopen(OBSURL)
...
class Notes:
    ...
    One Note object per frame
    """
    def __init__(self, all_stories):
        """
        :type all_stories: Stories
        """
        response = urllib.urlopen(NOTESURL)
        data = json.loads(response.read())
        for obj in data:
            if 'id' in obj:
...
class Terms(list):
    ...
    def __init__(self, all_stories):
        ...
        response = urllib.urlopen(TERMSURL)
        data = json.loads(response.read())
        term_id = 0
        for obj in data:
            if 'term' in obj:
